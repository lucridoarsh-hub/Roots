...
import MemoryModal from '../MemoryModel';
...
const Dashboard: React.FC = () => {
  ...
  // ========== STYLES (Dark Mode & High Contrast) ==========
  ...
  const borderColor = isDark ? 'rgba(255,255,255,0.05)' : theme.colors.gray[200];
  const topBarBg = isDark ? theme.colors.brand[950] + 'CC' : 'rgba(255,255,255,0.8)';
  const viewToggleBg = isDark ? theme.colors.brand[900] : theme.colors.gray[100];
  ...
  const viewToggleInactiveText = isDark ? theme.colors.brand[400] : theme.colors.gray[500];
  const addBtnBg = isDark ? theme.colors.brand[600] : theme.colors.brand[900];
  const addBtnHoverBg = isDark ? theme.colors.brand[500] : theme.colors.brand[800];
  const flashbackGradient = isDark
    ? 'linear-gradient(to right, #000000, #172554)'
    : 'linear-gradient(to right, #1e3a8a, #2563eb)';
  const memoryRestoreOverlay = isDark ? 'rgba(0,0,0,0.6)' : 'rgba(255,255,255,0.6)';
  const coverTitleColor = isDark ? theme.colors.white : theme.colors.gray[900];
  const coverDateColor = isDark ? theme.colors.gray[400] : theme.colors.gray[500];
  const coverDividerColor = isDark ? theme.colors.brand[700] : theme.colors.brand[900];
    ...
    <>
      ...
      <div
        style={{
          ...
          filter: isHighContrast ? 'contrast(1.25)' : 'none',
        }}
      >
        {/* Print Cover Page */}
        <div
