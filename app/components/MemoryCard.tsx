...
import theme from '../theme'; // ← New Enduring Roots theme (theme.ts / theme.js)
import {
  Calendar,
  Edit2,
  Trash2,
...
// ========== STAGE COLOR MAPPING (kept as-is – stage-specific) ==========
...
const MemoryCard: React.FC<MemoryCardProps> = ({
  ...
  // ========== NEW THEME COLORS (fully applied) ==========
  ...
  const overlayBg = isDark ? 'rgba(19, 34, 23, 0.95)' : 'rgba(250, 250, 249, 0.95)';
  const overlayText = textColor;

  const cancelBtnBg = isDark ? '#1f3824' : theme.light.bgInput;
  const cancelBtnHoverBg = isDark ? '#2f5237' : '#d9ede0';
  const cancelBtnText = textColor;

  const deleteBtnBg = theme.colors.rose[500];
  const deleteBtnHoverBg = '#e11d48';
  const deleteBtnText = '#ffffff';

  // Reaction colors – now using the new design system palette
  const heartColor = theme.colors.rose[500];
  const likeColor = theme.colors.brand[500];     // green = positive "like"
    ...
    <>
      ...
      >
        ...
        {showDeleteConfirm && (
          ...
          >
            <AlertCircle size={40} style={{ color: theme.colors.rose[500], marginBottom: theme.spacing(3) }} />
            <h4 style={{ fontWeight: 600, color: overlayText, marginBottom: theme.spacing(1), fontSize: theme.fontSize.lg }}>
              Delete this memory?
            </h4>
            <p style={{ fontSize: theme.fontSize.sm, color: textMuted, marginBottom: theme.spacing(5) }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: theme.spacing(3), paddingTop: theme.spacing(3), borderTop: `1px solid ${cardBorder}` }}>
            ...
            <div style={{ display: 'flex', gap: theme.spacing(1) }}>
              ...
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.backgroundColor = isDark ? 'rgba(244, 63, 94, 0.2)' : '#fee2e2';
                  e.currentTarget.style.color = deleteBtnBg;
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.backgroundColor = 'transparent';
                  e.currentTarget.style.color = textMuted;
